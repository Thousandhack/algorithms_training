...
自我总结：快速排序的步骤就是首先将第一个数作为两边排序的数值，
从第二个数开始对第一个数进行比较，如果大于第一个数将这个数放在第一个数的右边，
小于放左边，依次进行比较并存放，最后将队列分成三个部分: 左边都比第一个数值小的数，
第一个数值，右边都比第一各值大的数。这样就未完成了第一次排序的步骤。第二次是将前半部分的第一个值作为比较的”基数”，
同第一次排序进行同样的比较和存放，直到原队列的第一个值之前的那个值。右边和左边做同样的比较，排序，直到之后一个值。
这样依次进行递归，直到没有可以在分为左右的，这样就从小到大排序完成。
